...
plot(force_train[:, 3], force_train[:, 0], 'o', c="C2", ms = 5, label="Train dataset (RMSE={0:4.2f} mev/A)".format(loss[-1, 5]*1000))
plot(force_train[:, 4:6], force_train[:, 1:3], 'o', c="C2", ms = 5)
if test_flag == 1:
    plot(force_test[:, 3], force_test[:, 0], 'o', c="C7", ms = 2, label="Test dataset (RMSE={0:4.2f} mev/A)".format(loss[-1, 8]*1000))
