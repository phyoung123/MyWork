...
## round 5
鉴于 `round 4`的结果其实并没有达到最好，后期处理rdf时发现还是有偏差，所以我还是准备将 `round 4`的DFT加入到训练集中继续训练。`round 4`中的`10ns`NEP-MD本来只输出了`100`个结构，`round 4`只从`9-99`间隔`10`提取了`10`个结构。在本轮中，我还是将上述的`10`个结构加入训练集，并且重新加入了从`5-95`，间隔`10`个结构，又取了`10`个结构加入训练集。但是准备利用`pynep`将原来的一些数据给他踢出去，但是将主动学习的结构全保留。