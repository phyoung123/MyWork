...
# for i in 20 60 100 120
# do 

# cat >write_pressure.py <<EOF
# with open('Q.dat', 'a') as fo:
#     fo.write("$i'GPa'\nsupercell  Lx    mean_Q     mean_Q_error\n")
# EOF
# python write_pressure.py

# with open('../../../Q.dat', 'a') as fo:
#     fo.write("supercell  Lx    mean_Q     mean_Q_error\n")

# for j in 533  633 733 833 933
for j in 353 363 373 383 393
do
cd $j

cat > grep_lattice.py << EOF
import numpy as np
...
EOF
python grep_lattice.py


cat > eff_lattice.py <<EOF
import numpy as np
...
with open('../Q.dat', 'a') as fo:
    fo.write("{}  {}  {}  {}\n".format($j, Ly, mean_Q, mean_Q_error))
EOF
python eff_lattice.py
cd ..
done
